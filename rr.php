...
if(isset($_GET['q123']))
{
$sql123="Select  * from inters where JournalID=".$_GET['q123'];
...
<form id="message11" name="loginForm" action="#" enctype="multipart/form-data">
  <fieldset class="account-info">
   <legend>
   <h2>International Journal Details Update Form</h2></legend>
   ...
   <label>
    ...
    <select class="limitedNumbChosen" style="width: 450px; height: 30px; direction: ltr" dir="ltr" id="papertype11">
        ...
        if ($row123['18'] == "Original") {
            ...
            <option value="Case Report">Case Report</option>
            <?php
        } elseif ($row123['18'] == "Systematic Review") {
            ?>
  ...
  $(".limitedNumbChosen").chosen({
    ...
    <label>
      Publisher (Organization)
        <input type="text"  placeholder="Springer" required id="publisher11" value='<?php echo $row123['17'];?>'/>
    </label>
    ...
    <label>
      Date of Publication
<input type="date" name="journalyear11" placeholder="14-11-1984" id="journalyear11" value="<?= $row123[7];?>"/>
...
<input type="number" min="0.0" max="1000" step="0.1" name="IF" id="IF11" value='<?php echo $row123['8'];?>'/> 
   ...
   <label>
    Keywords
   <textarea id="multitext11" name="multitext11" rows="4" cols="50", placeholder="keyword1, keyword2"><?= $row123['20'];?></textarea>
</label>
...
<input type="number" min="1" max="1000" step="1" value='<?php echo $row123['10'];?>' id="no11" name="no"/> 
        ...
        <label>
      Number of Pages
<input type="text" placeholder="21 - 32" id="pages11" name="pages" value='<?php echo $row123['11'];?>'/> 
    </label>
...
<input type="text" placeholder="17/59/1234"  id="ethicalno11" value='<?php echo $row123['5'];?>' name="ethicalno"> 
    ...
    <label>
Country
 <select  class="limitedNumbChosen"style="width: 450px; height: 30px; direction: ltr" dir="ltr" id="country11" name="country">
   ...
   <?php
	...
	{
	?>
      <option value='<?php echo $row123['12'];?>' selected><?php echo $row123['12'];?></option>
...
<?php
	...
	?>
      ...
      value='<?php echo $row123['13'];?>' id="doi11">
    </label>
    
        <label>
      Weblink
      <input type="text" name="weblink" placeholder="http://www.example.com" id="weblink11" 
      value='<?php echo $row123['14'];?>'>
    </label>


<label>
    indexing
    <select class="limitedNumbChosen" style="width: 450px; height: 30px; direction: ltr" dir="ltr" id="indexing11">
        <?php
        if ($row123['19'] == "Scopus") {
            ?>
            ...
            <?php
        } elseif ($row123['19'] == "None Scopus") {
            ?>
            <option value="None Scopus" selected>None Scopus</option>
            <option value="Scopus">Scopus</option>
            <option value="Clarivate">Clarivate</option>
            <?php
        } else{
            ?>
            <option value="Clarivate" selected>Clarivate</option>
            <option value="None Scopus">None Scopus</option>
            <option value="Scopus">Scopus</option>
...
<script>
    ...
    <?php
$ethicFileLink1 = $row123[16]; // Assuming $row123[16] corresponds to the Ethic file link
...
function goToNewPage() {
  ...
  window.location.href = 'international_journal.php';
}
</script>
...
<script>
    ...
    var indexing11=$("#indexing11").val();
    var multitext11=$("#multitext11").val();

//   var fileInput = $('#file11')[0];
// var file11 = fileInput.files[0];
...
// var Lastfile11 = LastfileInput11.files[0];
...
var Ethicfile11 = EthicfileInput11.files.length > 0 ? EthicfileInput11.files[0] : null;
  ...
  var formData = new FormData();
   ...
   formData.append('authorplace11', authorplace11);
    ...
    formData.append('country11', country11);
    ...
    formData.append('indexing11', indexing11);
